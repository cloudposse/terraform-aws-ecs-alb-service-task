...
| <a name="input_bind_mount_volumes"></a> [bind\_mount\_volumes](#input\_bind\_mount\_volumes) | Task bind mount volume definitions as list of configuration objects. You can define multiple bind mount volumes on the same task definition. Requires `name` and optionally `host_path` | <pre>list(object({<br>    name      = string<br>    host_path = optional(string)<br>  }))</pre> | `[]` | no |
...
| <a name="input_docker_volumes"></a> [docker\_volumes](#input\_docker\_volumes) | Task docker volume definitions as list of configuration objects. You can define multiple Docker volumes on the same task definition, but a single volume can only have one `docker_volume_configuration`. | <pre>list(object({<br>    host_path = optional(string)<br>    name      = string<br>    docker_volume_configuration = list(object({<br>      autoprovision = optional(bool)<br>      driver        = string<br>      driver_opts   = optional(map(string))<br>      labels        = optional(map(string))<br>      scope         = optional(string)<br>    }))<br>  }))</pre> | `[]` | no |
