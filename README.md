...
| <a name="input_service_connect"></a> [service\_connect](#input\_service\_connect) | The connections to add to the service. See `connection` configuration block: https://www.terraform.io/docs/providers/aws/r/ecs_service.html#connection | <pre>object({<br>    cloud_map_namespace = string<br>    discovery_names = map(object({<br>      port_name     = string<br>      port_number   = number<br>      port_override = optional(number, null)<br>      dns_name      = optional(string)<br>    }))<br>  })</pre> | `null` | no |
