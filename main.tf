...
resource "aws_ecs_service" "ignore_changes_task_definition" {
  ...
  name                               = module.this.id
  task_definition                    = local.create_task_definition ? "${one(aws_ecs_task_definition.default[*].family)}:${join("", aws_ecs_task_definition.default[*].revision)}" : var.task_definition[0]
