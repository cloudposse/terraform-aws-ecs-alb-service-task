...
resource "aws_ecs_service" "ignore_changes_load_balancer" {
  count                              = local.ecs_service_enabled && var.ignore_load_balancer && !var.ignore_changes_task_definition && !var.ignore_changes_desired_count ? 1 : 0
  name                               = module.this.id
  task_definition                    = local.create_task_definition ? "${join("", aws_ecs_task_definition.default[*].family)}:${join("", aws_ecs_task_definition.default[*].revision)}" : var.task_definition[0]
  desired_count                      = var.desired_count
  deployment_maximum_percent         = var.deployment_maximum_percent
...
resource "aws_ecs_service" "ignore_changes_load_balancer_and_task_definition" {
  count                              = local.ecs_service_enabled && var.ignore_changes_load_balancer && var.ignore_changes_task_definition && !var.ignore_changes_desired_count ? 1 : 0
  name                               = module.this.id
  task_definition                    = local.create_task_definition ? "${join("", aws_ecs_task_definition.default[*].family)}:${join("", aws_ecs_task_definition.default[*].revision)}" : var.task_definition[0]
  desired_count                      = var.desired_count
  deployment_maximum_percent         = var.deployment_maximum_percent
...
resource "aws_ecs_service" "ignore_changes_load_balancer_and_desired_count" {
  count                              = local.ecs_service_enabled && var.ignore_changes_load_balancer && var.ignore_changes_desired_count && !var.ignore_changes_task_definition ? 1 : 0
  name                               = module.this.id
  task_definition                    = local.create_task_definition ? "${join("", aws_ecs_task_definition.default[*].family)}:${join("", aws_ecs_task_definition.default[*].revision)}" : var.task_definition[0]
  desired_count                      = var.desired_count
  deployment_maximum_percent         = var.deployment_maximum_percent
...
resource "aws_ecs_service" "ignore_changes_load_balancer_and_task_definition_and_desired_count" {
  count                              = local.ecs_service_enabled && var.ignore_changes_load_balancer && var.ignore_changes_task_definition && !var.ignore_changes_desired_count ? 1 : 0
  name                               = module.this.id
  task_definition                    = local.create_task_definition ? "${join("", aws_ecs_task_definition.default[*].family)}:${join("", aws_ecs_task_definition.default[*].revision)}" : var.task_definition[0]
  desired_count                      = var.desired_count
